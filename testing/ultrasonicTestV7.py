...
# ───────── LED STRIP (PWM1 / GPIO-13) ──────────────────────────────
LED_PIN, LED_CHAN = 13, 1                    # pin 33, PWM channel 1
strip = PixelStrip(NUM_LEDS, LED_PIN,
                   freq_hz = 800_000,
                   dma     = 10,             # any free DMA channel
                   invert  = False,
...
def strip_fill(r,g,b):
    c = Color(g,r,b);  [strip.setPixelColor(i, c) for i in range(NUM_LEDS)]
...
def ambient_fairway():
    for i in range(NUM_LEDS):
        strip_set(i, random.randint(60,180), 0, 0)     # shades of green
    strip_show()

def pulse_putt(col=(255,255,0), delay=0.05):
    r,g,b = col
    for pos in list(range(NUM_LEDS))+list(reversed(range(NUM_LEDS))):
        strip_fill(0,0,0)
        strip_set(pos,r,g,b); strip_show(); time.sleep(delay)

def paparazzi(run, density=0.25, flash_ms=60):
    end=time.time()+run; n=max(1,int(NUM_LEDS*density))
    while time.time()<end:
        strip_fill(0,0,0)
        for _ in range(n): strip_set(random.randrange(NUM_LEDS),255,255,255)
        strip_show(); time.sleep(flash_ms/1000)
