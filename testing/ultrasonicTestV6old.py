...
NUM_LEDS         = 150         # number of LEDs on strip
LED_BRIGHTNESS   = 0.6

AUDIO_DIR        = '/home/TestPi/golf_sounds'  # Combined taunts + claps
# ────────────────────────────────────────────────────────────────────────────
...
def ambient_fairway(frame):
    for i in range(NUM_LEDS):
        green_shade = random.randint(60, 180)  # broader range for realistic grass
...
def celebrate():
    ...
    # Start playing sound in a background thread
    audio_thread = threading.Thread(target=play_random_audio)
    audio_thread.start()

    # Begin light show immediately
    pulse_putt()
    paparazzi(max(0, CELEB_SEC - 2))

    # (Optional) If you wanted to wait for the sound, you could audio_thread.join() here
